...
function initLucky() {
    ...
    if (presentLuckyInfo) {
        if(presentLuckyInfo.startLeft>0){
            ...
            // 红包抢完
            //if (presentLuckyInfo.couponStock < 1) {
...
function initCommon() {
    ...
    var api = "/router/activity/award_list";
    ...
    $("#lucky-winning").click(function(){
        ...
        if(typeof luckData !='undefined'){
            ...
            $.get(api,{
                activityId: ACT_INFO.lucky.pre.activityId,
                activityType: ACT_INFO.lucky.pre.activityTypeId,
                t:Date.now()
            },function(res){
                var res=JSON.parse(res);
                if(res.status){
    ...
    $("#lottery-winning").click(function(){
        ...
        if(typeof lotteryData !='undefined'){
            ...
            $.get(api,{
                activityId: ACT_INFO.lottery.activityId,
                activityType: ACT_INFO.lottery.activityTypeId,
                t:Date.now()
            },function(res){
                var res=JSON.parse(res);
                if(res.status){
...
function getLayerContent(classN,tips,needClose,otherHtml){
    var classN=classN||'',close='<a href="javascript:void()" class="close">确     认</a>',otherHtml=otherHtml||'';
    if(!!needClose){
        close='';
    }
