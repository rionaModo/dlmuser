...
function zoomImg(obj){
    ...
    $(img).load(function() {
        ...
        ctrn.show();
        var cWidth=ctrn.find('.layui-layer-content').width();
        var style={};
        width / height > 1 ? (imgw = cWidth, imgh = cWidth * height / width, style['padding-top']=(imgw - imgh) / 2 + 'px' ) : (imgh = cWidth, imgw = cWidth * width / height, style['padding-left'],(imgh - imgw) / 2 + 'px');
        style.width=imgw;
        style.height=imgh;
...
(function(){
    ...
    function  getCoupon(){
        ...
        $.ajax({
            type: "POST",
            url: "/router/dealer/share_dealer_trading",
    ...
    function couponHandle(cop){
        ...
        if(!isEmpty(code[cop.status].addInfo)&&code[cop.status].addInfo==true){
            ...
            $img[0].src=isEmpty(coupon.dealerCouponDto.dealerCouponImgUrl)?(STATIC_PRE+'/resource/images/businesscoupon/default.jpg'):coupon.dealerCouponDto.dealerCouponImgUrl;
            $img.click(function(){
